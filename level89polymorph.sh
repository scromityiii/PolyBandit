...
# I don't think we need this - NG useradd level89_ans --create-home --password "$(openssl passwd -1 "password")" --shell /bin/bash --user-group
...
#The level variant identifier is commented under each possible elif condition. There are 16 variations, ranging from level89a to level89p.
...
if [ "$changer" == "1" ] || [ "$changer" == "2" ] || [ "$changer" == "3" ] || [ "$changer" == "4" ]
		then
		    bashfiletext="#this next line will cause the account to log out immediately" #level89a
       ...
       textfilename="please me.txt"
		...
		then
		    bashfiletext="#this next line will make the account logout and go kaput" #level89b
	...
	textfilename="open thisone.txt"
		...
		then
		    bashfiletext="#this next line is why you can't proceed into this account" #level89c
	...
	textfilename="me me.txt"
		...
		then 
		    bashfiletext="#the line below me blockades your path" #level89d
	...
	textfilename="in here.txt"
		...
		then 
		    bashfiletext="#destroy the line below me" #level89e
	...
	textfilename="in hereplease.txt"
		...
		then
		    bashfiletext="#eliminate the line below me (exit) and your way will be clear" #level89f
       ...
       textfilename="thisis it.txt"
		...
		then
		    bashfiletext="#if this next line is not deleted you won't proceed" #level89g
       ...
       textfilename="aperto aperto.txt"
		...
		then 
		    bashfiletext="#eliminate the line below and you shall pass" #level89h
	...
	textfilename="youare worthy.txt"
		...
		then 
		    bashfiletext="#delete the line that is below it makes the account log out" #level89i
		...
		then
		    bashfiletext="#want to pass? delete exit, it makes the account log out" #level89j
		...
		then
		    bashfiletext="#the line below immidiately causes the account to log out" #level89k
		...
		then 
		    bashfiletext="#in the line below this lies your obstacle to logging into this account" #level89l
		...
		then
		    bashfiletext="#in the next line is what blocks, it makes the account logout" #level89m
		...
		then
		    bashfiletext="#eliminate the next line, below, and you shall pass" #level89n
		...
		then
		    bashfiletext="#in the line below this lies a statement that causes this account to fail, delete it" #level89o
		...
		then
		    bashfiletext="#delete the line below this" #level89p
...
echo "*  For researchers: MD5 Hash first character: "$changer"                                *" >> /home/level89/README.txt;
echo "* You are at Level 89                                                            *" >> /home/level89/README.txt;
echo "*                                                                                *" >> /home/level89/README.txt;
echo "* The password for the next level is in a file called "$textfilename"            *" >> /home/level89/README.txt;
...
sed -i '1s/^/'"$bashfiletext"' /' /home/level89/.bashrc;
sed -i '2s/^/exit /' /home/level89/.bashrc;

echo $level90_pass > /home/level89/"$textfilename";
chown level89:level89 /home/level89/"$textfilename";
chown level89:level88 /home/level89/.bashrc;
chmod g+w /home/level89/.bashrc;

chmod 000 /home/level89/"$textfilename";
