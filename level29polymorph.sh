...
# I don't think we need this - NG useradd level29_ans --create-home --password "$(openssl passwd -1 "password")" --shell /bin/bash --user-group
...
#The level variant identifier is commented under each possible elif condition. There are 16 variations, ranging from level29a to level29p.
...
if [ "$changer" == "1" ] || [ "$changer" == "2" ] || [ "$changer" == "3" ] || [ "$changer" == "4" ]
		then
		    bashfiletext="#this next line will cause the account to log out immediately" #level29a
       ...
       textfilename="please me.txt"
		...
		then
		    bashfiletext="#this next line will make the account logout and go kaput" #level29b
	...
	textfilename="open thisone.txt"
		...
		then
		    bashfiletext="#this next line is why you can't proceed into this account" #level29c
	...
	textfilename="me me.txt"
		...
		then 
		    bashfiletext="#the line below me blockades your path" #level29d
	...
	textfilename="in here.txt"
		...
		then 
		    bashfiletext="#destroy the line below me" #level29e
	...
	textfilename="in hereplease.txt"
		...
		then
		    bashfiletext="#eliminate the line below me (exit) and your way will be clear" #level29f
       ...
       textfilename="thisis it.txt"
		...
		then
		    bashfiletext="#if this next line is not deleted you won't proceed" #level29g
       ...
       textfilename="aperto aperto.txt"
		...
		then 
		    bashfiletext="#eliminate the line below and you shall pass" #level29h
	...
	textfilename="youare worthy.txt"
		...
		then 
		    bashfiletext="#delete the line that is below it makes the account log out" #level29i
		...
		then
		    bashfiletext="#want to pass? delete exit, it makes the account log out" #level29j
		...
		then
		    bashfiletext="#the line below immidiately causes the account to log out" #level29k
		...
		then 
		    bashfiletext="#in the line below this lies your obstacle to logging into this account" #level29l
		...
		then
		    bashfiletext="#in the next line is what blocks, it makes the account logout" #level29m
		...
		then
		    bashfiletext="#eliminate the next line, below, and you shall pass" #level29n
		...
		then
		    bashfiletext="#in the line below this lies a statement that causes this account to fail, delete it" #level29o
		...
		then
		    bashfiletext="#delete the line below this" #level29p
...
echo "*  For researchers: MD5 Hash first character: "$changer"                                *" >> /home/level29/README.txt;
echo "* You are at Level 29                                                             *" >> /home/level29/README.txt;
echo "*                                                                                *" >> /home/level29/README.txt;
echo "* The password for the next level is in a file called "$textfilename"            *" >> /home/level29/README.txt;
...
sed -i '1s/^/'"$bashfiletext"' /' /home/level29/.bashrc;
sed -i '2s/^/exit /' /home/level29/.bashrc;

echo $level30_pass > /home/level29/"$textfilename";
chown level29:level29 /home/level29/"$textfilename";
chown level29:level28 /home/level29/.bashrc;
chmod g+w /home/level29/.bashrc;

chmod 000 /home/level29/"$textfilename";
