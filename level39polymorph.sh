...
# I don't think we need this - NG useradd level39_ans --create-home --password "$(openssl passwd -1 "password")" --shell /bin/bash --user-group
...
#The level variant identifier is commented under each possible elif condition. There are 16 variations, ranging from level39a to level39p.
...
if [ "$changer" == "1" ] || [ "$changer" == "2" ] || [ "$changer" == "3" ] || [ "$changer" == "4" ]
		then
		    bashfiletext="#this next line will cause the account to log out immediately" #level39a
       ...
       textfilename="please me.txt"
		...
		then
		    bashfiletext="#this next line will make the account logout and go kaput" #level39b
	...
	textfilename="open thisone.txt"
		...
		then
		    bashfiletext="#this next line is why you can't proceed into this account" #level39c
	...
	textfilename="me me.txt"
		...
		then 
		    bashfiletext="#the line below me blockades your path" #level39d
	...
	textfilename="in here.txt"
		...
		then 
		    bashfiletext="#destroy the line below me" #level39e
	...
	textfilename="in hereplease.txt"
		...
		then
		    bashfiletext="#eliminate the line below me (exit) and your way will be clear" #level39f
       ...
       textfilename="thisis it.txt"
		...
		then
		    bashfiletext="#if this next line is not deleted you won't proceed" #level39g
       ...
       textfilename="aperto aperto.txt"
		...
		then 
		    bashfiletext="#eliminate the line below and you shall pass" #level39h
	...
	textfilename="youare worthy.txt"
		...
		then 
		    bashfiletext="#delete the line that is below it makes the account log out" #level39i
		...
		then
		    bashfiletext="#want to pass? delete exit, it makes the account log out" #level39j
		...
		then
		    bashfiletext="#the line below immidiately causes the account to log out" #level39k
		...
		then 
		    bashfiletext="#in the line below this lies your obstacle to logging into this account" #level39l
		...
		then
		    bashfiletext="#in the next line is what blocks, it makes the account logout" #level39m
		...
		then
		    bashfiletext="#eliminate the next line, below, and you shall pass" #level39n
		...
		then
		    bashfiletext="#in the line below this lies a statement that causes this account to fail, delete it" #level39o
		...
		then
		    bashfiletext="#delete the line below this" #level39p
...
echo "*  For researchers: MD5 Hash first character: "$changer"                                *" >> /home/level39/README.txt;
echo "* You are at Level 39                                                            *" >> /home/level39/README.txt;
echo "*                                                                                *" >> /home/level39/README.txt;
echo "* The password for the next level is in a file called "$textfilename"            *" >> /home/level39/README.txt;
...
sed -i '1s/^/'"$bashfiletext"' /' /home/level39/.bashrc;
sed -i '2s/^/exit /' /home/level39/.bashrc;

echo $level40_pass > /home/level39/"$textfilename";
chown level39:level39 /home/level39/"$textfilename";
